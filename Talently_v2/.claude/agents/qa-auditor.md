...
| BR-S17 | El paso 1 de cada wizard (selección de tipo) BIFURCA al wizard del tipo elegido | En `useOnboardingCandidate.saveStep`: si `merged.user_type === 'company'` → `refreshProfile()` + `navigate('/onboarding/candidate'.opuesto)`. Espejo en `useOnboardingCompany`. Bug real 2026-06-11: usuario eligió "Soy Empresa" en el Step1 del wizard candidato y siguió llenando los 12 pasos de candidato con `user_type='company'`. El audit estático de routing guards (sec 12) NO lo cubría: los guards estaban bien, el flujo interno del wizard no. |
