...
| BR-S17 | La selección de tipo (candidato/empresa) existe UNA sola vez en toda la app y bifurca al wizard correcto | (a) El ÚNICO paso de selección de tipo es `candidate/Step1_TipoPerfil` — el wizard de empresa NO debe tener uno (la copia duplicada se eliminó el 2026-06-11 tras verse dos veces seguidas en el APK; grep `TipoPerfil` solo debe matchear el de candidato). (b) En `useOnboardingCandidate.saveStep`: si `merged.user_type === 'company'` → upsert SIN avanzar `onboarding_step` + `refreshProfile()` + `navigate('/onboarding/company')`. (c) `CompanyOnboarding` paso 1 con back → `/onboarding/candidate` para re-elegir. Bug original: usuario eligió "Soy Empresa" y siguió en el wizard de candidato; el audit de routing guards (sec 12) no cubría el flujo interno del wizard. |
| BR-S18 | El overlay de debug del errorLogger NO se muestra en producción | `logError` debe condicionar `showOverlay` a `overlayEnabled()` (DEV o flag `talently_debug` en localStorage). Bug real 2026-06-11: stack trace visible al equivocarse de contraseña. Además, errores ESPERADOS (credenciales inválidas, validaciones) se loguean con `overlay: false, level: 'warn'`. |
| BR-S19 | Toda ruta registrada en App.jsx tiene al menos un punto de entrada en la UI | Para cada `<Route path=...>`, grep de `navigate('<path>')` o `to="<path>"` en src/. Ruta sin entrada = pantalla fantasma (caso /app/settings 2026-06-11: soporte/FAQ/eliminar cuenta inaccesibles para candidatos). Excepciones: rutas de callback/deep-link (/auth/callback) y redirects (/dashboard). |
